...
| 2026-07-10 | Branding | App renamed **"B2B" → "Campus Crosswalk"**. "B2B" is retained as the umbrella brand for the ecosystem, NOT this app. Conventions: full "Campus Crosswalk" in titles/headers/emails/first mention; "Crosswalk" acceptable in-app after first mention; code slug `campus-crosswalk` (kebab) / `campus_crosswalk` (snake). Applied to UI strings, page titles, templates, verification emails, the consent flash, a CSS branding comment, and the `MAIL_FROM` default (`no-reply@campus-crosswalk.local`). NOT changed: DB table/column names, the `b2b.db` database filename (renaming would orphan existing local data), migration files, and this doc's earlier historical sections. Repo folder + GitHub remote rename to `campus-crosswalk` are being done manually outside the session. | UI/templates/emails/config; see this entry only — earlier sections unchanged |
